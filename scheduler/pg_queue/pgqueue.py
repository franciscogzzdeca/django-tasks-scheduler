...
def bulk_insert_events(curs, queue_name: str, rows: List[str], columns: List[str]):
...
class _Connection(object):
    ...
    def __init__(self, database: str, user: str, password: str, host: str, port: int, autocommit: bool = False,
                 appname: str = __file__) -> None:
        # allow override
        dsn = dict(
            database=quote_dsn_param(database),
            host=quote_dsn_param(host),
            user=quote_dsn_param(user),
            password=quote_dsn_param(password),
            port=str(port),
            # appname=quote_dsn_param(appname),
        )
...
class Ticker(_Connection):
    ...
    def __init__(self, database, user, password, host, port, config=None):
        if config is None:
            config = {}
        super(Ticker, self).__init__(database, user, password, host, port, autocommit=1, appname='PgQ Ticker')
    ...
    def check_pgq(self, curs):
        curs.execute("SELECT * FROM pg_catalog.pg_namespace n WHERE n.nspname='pgq';")
        res = curs.fetchone()
        if not res:
...
def _main(log_level=logging.INFO):
    ...
    ticker = Ticker(host='localhost', port=5432, database='devbb', user='devbb', password='devbb')
